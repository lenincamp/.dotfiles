...
local picker_status = require("modules.editor.picker.status")
...
function M.lines(opts, state)
  ...
  local title = state.prompt
    .. (state.current_query ~= "" and (" /" .. state.current_query) or "")
    .. (state.current_filter_label and (" [" .. state.current_filter_label .. "]") or "")
  ...
  local lines = {
    ...
    picker_display.padded_line(status_line, state.width),
  }
  ...
  for index = page_start, page_end do
    ...
    local label = picker_filter.item_label(item, opts)
    ...
    if state.current_query ~= "" then
      for _, pos in ipairs(picker_filter.match_positions(label, state.current_query)) do
