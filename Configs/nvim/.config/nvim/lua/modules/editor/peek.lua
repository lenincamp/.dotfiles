...
--- Open a location inside a chainable floating preview window.
--- @param loc table LSP location-like table
--- @param client_id number|nil
--- @param opts table|nil { word?: string, title?: string }
--- @return boolean
local function open_location(loc, client_id, opts)
    ...
    bmap("<CR>", function()
        close_all()
        if client_id then
            vim.lsp.util.show_document(loc, client_id, {
                focus = true
            })
            return
        end
...
local function request(method)
    if method == "textDocument/definition" then
        local ok_m, mybatis = pcall(require, "modules.editor.mybatis")
        if ok_m and type(mybatis.peek_definition) == "function" then
            local ok_h, handled = pcall(mybatis.peek_definition)
            if ok_h and handled then
                return
            end
        end
    end
    ...
    vim.lsp.buf_request(0, method, params, function(_, result, ctx)
        ...
        open_location(loc, ctx and ctx.client_id or nil, {
            word = word
        })
    end)
end

--- Preview any file location using the same chainable peek UI as gpd.
--- @param location table { filename?: string, uri?: string, lnum?: number, col?: number, title?: string }
--- @return boolean
...
-- ── References via native quickfix ─────────────────────────────────────

local function references()
    require("modules.editor.lsp_lists").references()
end
