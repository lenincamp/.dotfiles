...
function M.items()
  ...
  return {
    ...
    action("Recent files", "Files", function() p.recent_files({ title = "Recent Files" }) end),
    action("Recent files (cwd)", "Files", function() p.recent_files({ cwd = vim.fn.getcwd(), title = "Recent Files (cwd)" }) end),
    ...
    action("Grep regex (root)", "Search", function() p.grep({ cwd = p.root(), regex = true, title = "Grep Regex (root)" }) end),
    action("Grep ignored literal", "Search", function() p.grep({ ignored = true, regex = false, title = "Grep Ignored Literal" }) end),
