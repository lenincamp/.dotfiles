...
local function build_palette(mode)
  ...
  local border = read_hl("FloatBorder")
  ...
  return {
    ...
    selection = first_hex(visual.bg, diff_add.bg) or (dark and "#313244" or "#ccd0da"),
    ok = first_hex(diff_add.fg, keyword.fg) or (dark and "#a6e3a1" or "#40a02b"),
    warn = first_hex(diag_warn.fg, title.fg) or (dark and "#f9e2af" or "#df8e1d"),
    error = first_hex(diag_error.fg, diff_delete.fg) or (dark and "#f38ba8" or "#d20f39"),
  }
end
...
local function write_lazygit_generated_config(path, palette)
  local lines = {
    ...
    "git:",
    "  pagers:",
    "    - pager: delta --paging=never --line-numbers --config delta.side-by-side=false",
  }
...
local function write_delta_generated_config(theme_key, palette)
...
function M.sync_git_delta_theme(theme)
  ...
  apply_delta_generated_palette(item.key, palette_colors)
end
...
function M.sync_lazygit_theme(theme)
  ...
  local ok_write = pcall(write_lazygit_generated_config, generated, palette)
...
local function iterm2_mode_matches(mode)
  ...
  if is_light == nil then return false end
...
local function iterm2_apply_high_contrast_mode(mode)
  ...
  vim.fn.system({ "osascript", "-e", script })
  if vim.v.shell_error ~= 0 then return false end
  return iterm2_mode_matches(mode)
end
...
function M.sync_iterm2_theme(theme)
  ...
  local mode_ok = preset_applied and iterm2_mode_matches(mode)
...
function M.request_external_sync(theme)
  M._external_sync.theme = M.resolve(theme or vim.g.pure_colorscheme or vim.g.colors_name or M.default)
...
function M.setup_autocmd()
  ...
  vim.api.nvim_create_autocmd("ColorScheme", {
    group = group,
    callback = function()
      if vim.g._pure_applying_colorscheme then return end
